...
if (-not (Test-Path -LiteralPath $CanonicalScript)) {
  Write-Error "Don Design Partner Kit wrapper: canonical script not found. The canonical Design Partner Kit lives at design_partner_bundle. Run design_partner_bundle\verify_all.ps1 from the repository root. Expected path: $CanonicalScript"
  exit 1
}
