...
```typescript
// ❌ BAD
logger.info("API key", { api_key: "sk_live_REDACTED" });

// ✅ GOOD (will be redacted automatically, but still avoid)
logger.info("API call", { api_key: "sk_live_REDACTED" }); // Redacted to [REDACTED]
...
const API_KEY = "sk_live_REDACTED_DO_NOT_COMMIT";
