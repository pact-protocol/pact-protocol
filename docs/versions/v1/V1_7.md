...
```bash
pnpm demo:v1:happy -- --settlementProvider stripe_like --stripeAsync --saveTranscript
```

**Async settlement that fails:**
```bash
pnpm demo:v1:happy -- --settlementProvider stripe_like --stripeAsync --stripeFail --saveTranscript
```

**Synchronous (default):**
```bash
pnpm demo:v1:happy -- --settlementProvider stripe_like
```
