...
- **Batch anchoring:** Batch anchoring (e.g., Merkle digests) is out-of-scope for v4.0.5-rc3 and is not shipped.
...
5. **Tier metadata MUST NOT be used to adjust insurer coverage outcomes in v4.x.**
6. **Absence of tier metadata.** If tier metadata is absent, verification and admissibility are unchanged; no tier is inferred.
...
- **Shipped:** Tier and SLA metadata may appear in GC View and insurer summary as additive fields (see [ADDITIVE_FIELD_WHITELIST_v4x.md](./ADDITIVE_FIELD_WHITELIST_v4x.md)). Evidence Viewer may display them for audit scheduling context only.
- **Out of scope:** Batch anchoring (e.g., Merkle digests) is out-of-scope for v4.0.5-rc3 and is not shipped.
