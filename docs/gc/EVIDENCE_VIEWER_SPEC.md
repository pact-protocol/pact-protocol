...
The viewer operates on **Auditor Packs** (`.zip` files) generated by `pact-verifier auditor-pack`. It extracts and displays structured information from the pack's JSON artifacts, presenting it in panels optimized for different review workflows. Optionally, the viewer can load a **passport snapshot** (JSON produced by **Boxer** from the same pack + an anchors file); the snapshot is **derived, not evidence** and supplies identity badges, trust signals, and revocation warnings. See [BOXER.md](../BOXER.md).
